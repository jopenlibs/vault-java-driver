...
public class Pki extends OperationsBase {
     ...
     * being present in the <code>VaultConfig</code> instance.</p>
     *
     * <p>This version of the method accepts a <code>RoleOptions</code> parameter, containing
     * optional settings for the role creation operation.  Example usage:</p>
     *
     * <blockquote>
     * <pre>{@code
     * final VaultConfig config = new VaultConfig.address(...).token(...).build();
     ...
     *
     * @param roleName A name for the role to be created or updated
     * @param options Optional settings for the role to be created or updated (e.g. allowed domains,
     * ttl, etc)
     * @return A container for the information returned by Vault
     * @throws VaultException If any error occurs or unexpected response is received from Vault
     */
    public PkiResponse createOrUpdateRole(final String roleName, final RoleOptions options)
    ...
    /**
     * <p>Operation to retrieve an role using the PKI backend.  Relies on an authentication token
     * being present in the <code>VaultConfig</code> instance.</p>
     *
     * <p>The role information will be populated in the <code>roleOptions</code> field of the
     * <code>PkiResponse</code> return value.  Example usage:</p>
     *
     * <blockquote>
     * <pre>{@code
     * final VaultConfig config = new VaultConfig.address(...).token(...).build();
    ...
    /**
     * <p>Operation to delete an role using the PKI backend.  Relies on an authentication token
     * being present in the <code>VaultConfig</code> instance.</p>
     *
     * <p>A successful operation will return a 204 HTTP status.  A <code>VaultException</code> will
     * be thrown if the role does not exist, or if any other problem occurs.  Example usage:</p>
    ...
    /**
     * <p>Operation to generate a new set of credentials or sign the embedded CSR, in the PKI
     * backend. If CSR is passed the sign function of the vault will be called if not, issue will be
     * used. The issuing CA certificate is returned as well, so that only the root CA need be in a
     * client's trust store.</p>
     *
     * <p>A successful operation will return a 204 HTTP status.  A <code>VaultException</code> will
     * be thrown if the role does not exist, or if any other problem occurs.  Credential information
     * will be populated in the
     * <code>credential</code> field of the <code>PkiResponse</code> return value.  Example
     * usage:</p>
     *
     * <blockquote>
     * <pre>{@code
    ...
    private String roleOptionsToJson(final RoleOptions options) {
        ...
        if (options != null) {
            ...
            addJsonFieldIfNotNull(jsonObject, "email_protection_flag",
                    options.getEmailProtectionFlag());
