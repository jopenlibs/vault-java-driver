...
public class VaultContainer extends GenericContainer<VaultContainer> implements TestConstants,
    ...
    public VaultContainer(String image) {
        ...
        this.withNetwork(CONTAINER_NETWORK)
                ...
                .withClasspathResourceMapping("/startup.sh", CONTAINER_STARTUP_SCRIPT,
                        BindMode.READ_ONLY)
                .withClasspathResourceMapping("/config.json", CONTAINER_CONFIG_FILE,
                        BindMode.READ_ONLY)
                .withClasspathResourceMapping("/libressl.conf", CONTAINER_OPENSSL_CONFIG_FILE,
                        BindMode.READ_ONLY)
                .withClasspathResourceMapping("/approlePolicy.hcl", APPROLE_POLICY_FILE,
                        BindMode.READ_ONLY)
    ...
    /**
     * To be called by a test class method annotated with {@link org.junit.BeforeClass}. This logic
     * doesn't work when placed inside of the constructor, presumably because the Docker container
     * spawned by TestContainers is not ready to accept commands until after those methods
     * complete.
     *
     ...
     * the backend and populates test data).
     */
    ...
    public void setupBackendAppRole() throws IOException, InterruptedException {
        ...
        runCommand("vault", "policy", "write", "-ca-cert=" + CONTAINER_CERT_PEMFILE, "testerrole",
                APPROLE_POLICY_FILE);
    ...
    /**
     * Prepares the Vault server for testing of the PKI auth backend (i.e. mounts the backend and
     * populates test data).
     */
    public void setupBackendPki() throws IOException, InterruptedException {
        runCommand("vault", "login", "-ca-cert=" + CONTAINER_CERT_PEMFILE, rootToken);

        runCommand("vault", "secrets", "enable", "-ca-cert=" + CONTAINER_CERT_PEMFILE, "-path=pki",
                "pki");
    ...
    /**
     * <p>Constructs an instance of the Vault driver, providing maximum flexibility to control all
     * options explicitly.</p>
     *
     * <p>If <code>maxRetries</code> and <code>retryMillis</code> are BOTH null, then the
     * <code>Vault</code>
     * instance will be constructed with retry logic disabled.  If one OR the other are null, the
     * the class-level default value will be used in place of the missing one.</p>
     */
    ...
    public Vault getVault(final String token) throws VaultException {
        final VaultConfig config =
                new VaultConfig()
                        ...
                        .sslConfig(new SslConfig().pemFile(new File(CERT_PEMFILE)).build())
                        .build();
        return new VaultImpl(config).withRetries(MAX_RETRIES, RETRY_MILLIS);
    }
    ...
    /**
     * Returns the master key for unsealing the Vault instance.  This method should really ONLY be
     * used by tests specifically for sealing and unsealing functionality (i.e. SealTests.java).
     * Generally, tests should retrieve Vault instances from the "getVault(...)" or "getRootVault()"
     * methods here, and never directly concern themselves with the root token or unseal key at
     * all.
     *
